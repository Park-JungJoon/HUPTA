...
def ts_download_parser(tissue, ts_score, consititive_score, consensus_percentile):  #return tab separated list
    ...
    for k,v in main_dict.items():
        if v["TS score"] == 'Not Detected' or v["TS score"] == "NA":
            continue
...
def file_write(my_list,string):
    random_number = random.randint(1000, 9999)
    current_date = datetime.now().strftime('%Y_%m_%d_%H_%M:%S')
    filename = f"{string}_{current_date}_{random_number}.txt"
    root = media + '/tmp/'
